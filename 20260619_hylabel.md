...
- **목적**: PyQt6 기반 이미지 라벨링 GUI 프로그램 (LabelMe JSON 출력)
...
└── labeler/
    ...
    └── coco_io.py          # LabelMe JSON 저장/불러오기
```
...
- **형식**: LabelMe JSON (segmentation polygon 형식)
...
class MaskManager:
    ...
    load_from_coco(annotations)           # LabelMe polygon → 마스크 래스터화
```
