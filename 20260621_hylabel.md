...
**스택**: PyQt6 · NumPy · OpenCV · LabelMe JSON

이미지 라벨링 앱. 폴리곤/브러쉬로 mask를 그리고 Enter 키로 개별 annotation으로 커밋. LabelMe JSON 형식으로 저장.
...
├── main.py
├── run_hylabel.py
├── verify_coco.py       # LabelMe 저장·로드 검증 스크립트
├── requirements.txt
├── .gitignore
└── labeler/
    ├── __init__.py
...
## LabelMe 저장·로드 검증 (`verify_coco.py`)

### 검증 항목 (30/30 통과)

| 항목 | 내용 |
|---|---|
| annotation 개수 | 3개 annotation → 3개 LabelMe entry 생성 |
